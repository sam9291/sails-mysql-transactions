...
Transaction = function (callback) {
    ...
    /**
     * Store the state the transaction object is in.
     * Possible states are:
     *     - initialsed
     *     - started
     *     - committed
...
util.extend(Transaction, /** @lends Transaction */ {
    ...
    associateConnection: function (connection, uid, config) {
        if (!uid) {
            throw new AdapterError(AdapterError.TRANSACTION_ID_NOT_FOUND);
        }

        if (!config) {
            throw new AdapterError(AdapterError.TRANSACTION_CONFIG_NOT_FOUND);
        }
...
util.extend(Transaction.prototype, /** @lends Transaction.prototype */ {
    ...
    commit: function (callback) {
        ...
        // if the transaction has already been committed or rolledback, return an error
        // a transaction can only be committed if the state is either initialised or started
        if (self._state !== 'started' || self._state !== 'initialised') {
            return callback && callback(new AdapterError(AdapterError.TRANSACTION_INVALID_STATE_COMM));
        }

        // if commit was called with no active conn, it implies, no transact action
        // was called. as such it is an error.
        if (!conn) {
            return callback && callback(new AdapterError(AdapterError.TRANSACTION_UNINITIATED_COMM));
        }
    ...
    rollback: function (callback) {
        ...
        // if the transaction has already been committed or rolledback, return an error
        // a transaction can only be rolledback if the state is either initialised or started
        if (self._state !== 'started' || self._state !== 'initialised') {
            return callback && callback(new AdapterError(AdapterError.TRANSACTION_INVALID_STATE_ROLL));
        }

        // if commit was called with no active conn, it implies, no transact action
        // was called. as such it is an error.
        if (!conn) {
            return callback && callback(new AdapterError(AdapterError.TRANSACTION_UNINITIATED_ROLL));
        }
    ...
    toString: function () {
        return '[object Transaction:' + (this.connection() && this.connection().transactionId || this._state) + ']';
    }
});
